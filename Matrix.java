...
public class Matrix
{
	...
	public void copyBlock(int destRow, int destCol, Matrix that, int rowBegin, int colBegin, int rowCount, int colCount)
	{
        ...
        // Save fold rows to outputFoldMatrix and the remaining rows to train with outputTrainMatrix using this matrix.
        public void copyFoldBlock(Matrix outputFoldMatrix, Matrix outputTrainMatrix, int foldcount, int j)
	{
            int foldRowPosition = 0;
            int trainRowPosition = 0;
            int foldLowerBound = j*foldcount;
            int foldUpperBound = foldLowerBound+foldcount;
            for(int i = 0; i < this.rows(); i++) {
                if(i < foldLowerBound || i >= foldUpperBound) {
                    System.arraycopy(this.row(i), 0, outputTrainMatrix.row(trainRowPosition), 0, this.cols());
                    trainRowPosition++;
                }
                else {
                    System.arraycopy(this.row(i), 0, outputFoldMatrix.row(foldRowPosition), 0, this.cols());
                    foldRowPosition++;
                }
            }
	}
        
        // Leave-One-Out Cross Validation, copy all but one column.
        public void copyLOOBlock(Matrix fullMatrix, int colLeaveOut, int[] oneOut)
	{
            int colPosition = 0;
            
            for(int i = 0; i < fullMatrix.cols(); i++) {
                int check = i;
                if(!IntStream.of(oneOut).anyMatch(x -> x == check)) {
                    if(colPosition != colLeaveOut) {
                        //System.arraycopy(this.row(i), 0, outputTrainMatrix.row(trainRowPosition), 0, this.cols());
                        this.copyBlock(0, colPosition, fullMatrix, 0, i, fullMatrix.rows(), 1);
                        colPosition++;
                    }
                    else {
                        colLeaveOut--;
